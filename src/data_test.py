...
def pad_list(xs):
    ...
    if len(max_len) == 1:
        pad = np.zeros((n_batch, max_len[0]))
    else:
        pad = np.zeros((n_batch, max_len[0], max_len[1]))
    pad = Tensor.from_numpy(pad)
...
class DatasetGenerator:
        ...
        if segment >= 0.0:
            ...
            while True:
                ...
                if len(part_mix) > 0:
                    ...
                    mixtures_pad = ops.Cast()(mixtures_pad, mindspore.float32)
                    sources_pad = ops.Cast()(sources_pad, mindspore.float32)
                    for i in range(len(mixtures_pad)):
                        mixture_pad.append(mixtures_pad[i])
                        lens.append(ilens[i])
                        source_pad.append(sources_pad[i])
                    # mixture_pad.append(mixtures_pad)
                    # lens.append(ilens)
                    # source_pad.append(sources_pad)
    ...
    def sort_and_pad(self, batch):
        ...
        mixtures_pad = pad_list([Tensor.from_numpy(mix)
                                 for mix in mixtures])
        ilens = Tensor.from_numpy(ilens)
        sources_pad = pad_list([Tensor.from_numpy(s)
                                for s in sources])
        # N x T x C -> N x C x T
        transpose = ops.Transpose()
        sources_pad = ops.Cast()(sources_pad, mindspore.float32)
        sources_pad = transpose(sources_pad, (0, 2, 1))
